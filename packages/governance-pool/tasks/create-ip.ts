...
export default task(
    "create-ip",
    "Create investment pool for frontend on goerli testnet from InvestmentPoolFactory"
)
    .addParam("factory", "Investment pool factory address")
    .setAction(async ({factory}, hre) => {
        if (!availableTestnetChains.includes(hre.network.name)) {
        ...
        const seedFundingLimit: BigNumber = ethers.utils.parseEther("500");
        ...
        investmentPoolFactory = await ethers.getContractAt("InvestmentPoolFactory", factory);
